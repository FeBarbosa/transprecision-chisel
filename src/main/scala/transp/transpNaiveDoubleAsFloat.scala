...
class transpNaiveDoublAsFloat extends Module{
  ...
  val isFMV_WX = (inst_transp(31, 25) === 120.asUInt(7.W) && inst_transp(24, 20) === 0.asUInt(5.W) && inst_transp(14, 12) === 0.asUInt(3.W) && inst_transp(6,0) === 83.asUInt(7.W))
  ...
  // when(isFLD) {
  //   printf("GGG")
  //   printf("\nDtoF_dmem_resp_data.io.output = %b\n", DtoF_dmem_resp_data.io.output)
  //   printf("io.data_word_bypass = %b\n", io.data_word_bypass)
  // }

  // FCVT_DS is temporarily working as normal (double instruction)
  when((fmt === 1.asUInt(2.W) || isFCVT_SD || isFLD || isFSD) && (!isFLW && !isFSW) && !isFCVT_DS && !isFMV_WX)
  {
    // exclude FCVT.S.D, FCVT.D.S, FLD, FSD, FMV_XD and FMV_DX
    //when(!isFLD && !isFSD && !isFCVT_SD && !isFCVT_DS && !isFMV_XD && !isFMV_DX) {
    when(!isFLD && !isFSD && !isFCVT_SD && !isFMV_XD && !isFMV_DX) {
