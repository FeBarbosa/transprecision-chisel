...
class Rocket(implicit p: Parameters) extends CoreModule()(p)
  ...
  // TRANSPRECISION MODULE
  val transpNaive = Module(new transpNaiveDoublAsFloat())
  ...
  when (mem_pc_valid) {
    ...
    // wb_reg_wdata := Mux(!mem_reg_xcpt && mem_ctrl.fp && mem_ctrl.wxd, io.fpu.toint_data, mem_int_wdata) // Normal
    wb_reg_wdata := Mux(!mem_reg_xcpt && mem_ctrl.fp && mem_ctrl.wxd, transpNaive.io.res_toint_data, mem_int_wdata) // Transp
  ...
  transpNaive.io.id_inst := id_inst(0)
  transpNaive.io.data_word_bypass := io.dmem.resp.bits.data_word_bypass
  transpNaive.io.store_data := io.fpu.store_data
  transpNaive.io.toint_data := io.fpu.toint_data
  transpNaive.io.ex_rs0 := ex_rs(0)
  transpNaive.io.dmem_resp_bits_typ := io.dmem.resp.bits.typ

  io.fpu.inst := transpNaive.io.inst
  io.fpu.dmem_resp_data := transpNaive.io.dmem_resp_data
  io.fpu.dmem_resp_type := transpNaive.io.dmem_resp_type
  io.fpu.fromint_data := transpNaive.io.res_fromint_data
  io.dmem.s1_data.data := (if (fLen == 0) mem_reg_rs2 else Mux(mem_ctrl.fp, Fill((xLen max fLen) / fLen, transpNaive.io.resFPU), mem_reg_rs2)) // transp
  // ------------------------------------------------------------------------------------------------------------------
  ...
  if (enableCommitLog) {
    ...
    printf("C%d: %d [%d] pc=[%x] W[r%d=%x][%d] R[r%d=%x] R[r%d=%x] inst=[%x] DASM(%x)\n",
         ...
         coreMonitorBundle.inst, coreMonitorBundle.inst) 
          ...
          // printf("\nio.fpu.inst = %b\n", io.fpu.inst)
          // printf("io.fpu.fromint_data = %b\n", io.fpu.fromint_data)
	  // printf("io.fpu.toint_data = %b\n", io.fpu.toint_data)
          // printf("io.fpu.dmem_resp_data = %b\n", io.fpu.dmem_resp_data)
          // printf("io.fpu.dmem_resp_type = %b\n", io.fpu.dmem_resp_type)
          // printf("io.dmem.s1_data.data = %b\n\n", io.dmem.s1_data.data)
  }
